...
export class FishSimulation {
  ...
  public async initialize(): Promise<void> {
    ...
    if (this.tagContainer) {
      // Set up tag renderer
      this.tagRenderer.setSize(
        this.tagContainer.clientWidth,
        this.tagContainer.clientHeight
      );
      ...
      // Improve lighting for better tag visibility
      this.tagScene.background = new THREE.Color(0x222222); // Slightly lighter background

      // Add stronger lighting
      const tagAmbientLight = new THREE.AmbientLight(0xaaaaaa, 1.0); // Brighter ambient light
      this.tagScene.add(tagAmbientLight);

      const tagDirectionalLight = new THREE.DirectionalLight(0xffffff, 1.0); // Brighter directional light
  ...
  private createSimpleFishRobot(): void {
    ...
    for (let i = 0; i < numJoints; i++) {
      ...
      // Create a copy for the tag scene
      if (this.tagContainer) {
        const tagLineGeometry = new THREE.BufferGeometry().setFromPoints(
          points
        );
  ...
  private createAprilTag(id: number, position: THREE.Vector3): void {
    // Create a canvas for the AprilTag with higher contrast
    const canvas = document.createElement("canvas");
    canvas.width = 128; // Larger size for better visibility
    canvas.height = 128;
    const ctx = canvas.getContext("2d");

    if (!ctx) return;

    // Draw AprilTag pattern with higher contrast
    ...
    // Pure white inner square
    ctx.fillStyle = "#ffffff";
    ctx.fillRect(32, 32, 64, 64);

    // Draw ID number in the center with high contrast
    ctx.fillStyle = "#000000";
    ...
    sprite.position.y += 0.3; // Position higher above the joint for better visibility
    sprite.scale.set(0.8, 0.8, 1); // Make it larger
  ...
  private updateTagView(): void {
    ...
    this.tagScene.traverse((object) => {
      if (
        ...
        lineIndex < this.fishLines.length
      ) {
  ...
  public dispose(): void {
    ...
    if (this.tagRenderer.domElement.parentNode) {
      this.tagRenderer.domElement.parentNode.removeChild(
        this.tagRenderer.domElement
      );
    }
